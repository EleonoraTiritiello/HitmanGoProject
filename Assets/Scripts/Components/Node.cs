...
namespace HitmanGO
{
    ...
    public class Node : MonoBehaviour
    {
        ...
        /// <summary>
        /// Check if this <c> Node </c> is connected to a given <c> Node </c>
        /// </summary>
        /// <param name="node"> A given <c> Node </c> </param>
        /// <returns> Returns <c> true </c> if a <c> Connection </c> exists, otherwise returns <c> false </c> </returns>
        public bool VerifyConnection(Node node)
        {
            foreach(Connection connection in Connections)
            {
